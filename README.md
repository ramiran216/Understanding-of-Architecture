# [1]Understanding-of-Architecture of llm part
大模型时代，应用应该如何架构？本文依然关注“道”层面。同时也是笔者的一个知识迁移过程的一些感悟，先记录后整理。

## WHY
【~2024年5月】笔者认为大模型时代应该抛弃一些传统的架构思维，才能充分发挥大模型的作用，而不是机械的用大模型来代替原有系统中的某几项，就算是应用了大模型。显然当前大多数同行们还没有这样想。
当然我也理解，所谓肌肉记忆也行，船大难掉头也行，归根结底一句话终局思维下：马车上装引擎，只是阶段性产物。
【~2026年3月】Agent建设过程中，上下文工程尤为重要，包括记忆管理。

# [2]Understanding-of-Architecture of biz-arch part
...
当然可能大部分技术同学目前暂时没有开始做架构方面的工作，也没问题，按照我的理解，大家的成长阶段可简略概括为：```做好别人让自己干的任务 → 主动发现问题并自己实现完成 → 做好上级给自己的“一句话”任务 → 争取上级的资源做自己发现的事情```。无论在哪个阶段，认识和理解架构思维，相信对工作和成长都是受益的。

这是一个长跑，也是一种修行吧！

## 零、开篇
笔者从事软件行业已满16年了，回想整个行业的发展历程，真的是快到可怕，时至今日笔者觉得绝大部分从业者虽然在Coding，但某种意义上已经不是技术人员了。为什么这么说？笔者理解的技术人员是面对“不确定性”，解决“规模化”问题的一群人，按照这个标准，相信已经把很多人划出界外，因为在云计算快速普惠的时代，从纯技术层面已经接近解决。
...
5. 合并统一需要的是专业架构处理能力，分拆发展需要的是全局智慧格局思想。
...
6. 架构师一定要考虑```学强扶弱```，比如设计平台型架构的时候要积极沉淀先进应用的知识回平台层，然后帮助另一些初创型应用快速掌握&利用好这些先进知识。当然先进反哺需要一些机制的保障。
...
2. 围绕【核心资源】展开架构工作，比如流量入口，客户触达出口，偏底层技术支撑层，交付流水线，统一数据中心等等，要站在上帝视角看软件开发工作，用技术（解决规模性问题）方法解决。
...
7. **要有技术格局**，技术格局通俗的来说就是站在更高一层看问题，别总盯着自己的一亩三分地。举个例子笔者与一个主导做云原生应用管理平台的架构师合作，笔者所在的是ToB业务部门，面对ToB客户更多定制化的需求场景提出了类似计算机主板的架构方法：编排业务组件，开放应用API，配合OEM方式做客户的软件私有化交付业务，这期间该架构师认为我们在应用层做了系统层的事情。笔者想说的有两点：一是做平台要留有余地，要打造繁荣的生态需要开放的心态；二是很多事情是先在业务层落地应用，然后再抽象沉淀到平台层的。
8. ***保持简单***，大家都知道一个俗语：加一层能解决大部分的分工问题，有时候这个方法也确实很有用。但笔者觉得这种招儿得用在刀刃上，用多了看起来是解耦了，带来的问题更大。```Less is More```，要尽全力保持架构让人看得懂，改的动。
9. ***不要过度抽象***，通常来说抽象归纳能力是架构工作的一个必备素质，我们寻求的是解决问题的通用模型，而不是一个一个的Case方法，这是价值。同时也要避免过度的抽象，过犹不及这句老话在这里是生效的。举个例子Java当中的Object很通用吧，但绝大部分时候不能直接使用，需要在上面增加特定的行为属性。还是那句话设计有时候要折中落地一些，或是因为权衡工程性能，或是因为实现复杂度。
...
7. ***全局最优***高于***局部最优***，架构工作就是在保持技术先进性的条件下找寻全局最优解，并不断迭代修正的过程。

#### 提升基线（坚实Support），技术普惠（保持自由度）
...
5. 相比框架和库的轻资产模式，平台和服务是重资产，需要配套强运维才能确保高SLA。所以是否新增一个平台和服务中非常重要的一个因素是是否有足够的配套运维资源。曾经有一段时间，笔者工作的团队中出现了一个研发同学手上有2.5个平台or服务的情况，导致这个研发同学是白天解答客户问题，晚上修BUG and 开发新功能。可想而知这种情况下的服务质量，长此以往研发同学崩溃是必然的。平台和服务首先需要有核心竞争力来争取客户的采买，请记住这不是一锤子买卖，如果不能有保证高质量的“售后”加持，那选择用框架和LIB的方式来赋能，或许对双方都是更好的选择。

## 附录一、对于大热的DDD的评价
1. DDD是一个复杂业务的落地项目合作框架，指导和约定从业务到技术的基本关键点。
2. DDD在代码层面的指导意义不大，在业务组件（微服务）粒度应用的性价比最高。
3. 从复杂多变的业务中分离出稳定的业务or技术层，这才是本质，DDD在这方面只是个方法。
4. 行业中以阿里星环为首的业务PaaS平台的设计主张用DDD方法论来指导业务逻辑拆分，用SPI的方式来织入Domain Logic和App Logic。但实际落地的时候更多的是降为App Logic的框架化工作了，比如直接把一些参数校验的工作给拆分出来了，KPI是可以达成了，但不见得有那么重大的收益。
...
3. 资深一些的架构师，建议把精力多花在业务架构、数据架构、应用架构的设计和优化上，做好架构治理工作，这些事情的影响面更大更重要。
4. 建议不要扔下这个技能，至少还能辅导你的小朋友呢，技多不压身嘛。
...
4. 当前阶段的DevOps工具还都没有跟上云原生，坦白说没有跟上微服务架构。举个例子：拿代码仓库来说，现在是否有产品主Repo，且主Repo中应该都是一些应用级的Configuration；拿CICD Pipeline来说，是否已经有【集成流水线】来统一支持协调多个子模块/微服务。

## 附录十、如何做好ToB业务的架构工作
1. 坚守以客户为中心、以客户成功为导向的价值理念。很多技术同学以为以客户为中心是个业务口号，这肯定是不对的。笔者理解这句话的本质意思是要站在客户的角度思考，解决客户的问题，帮助客户的成长。
2. ToB是复杂业务，对这个是有行业通用认知的。要解决复杂问题，就需要总体设计、总体部署，决不能像ToC那样从简版入手（这里的“简版”指的是设计上的简，并非是客户产品UI上的简）。要比客户更了解业务，要用技术手段支持客户的个性化需求，才能叫以客户为中心。
3. 不要羡慕C端应用的高并发、海量数据等，把心思花在可配置、可扩展、高度可定制、系统安全、高效交付等方面，这里也是一片开阔地。

## 附录十一、互联网两大类架构
1. 以大搜索平台为代表的ToC的技术架构，是流量分配的工程架构 + 核心算法模块的支持。这里主要是按照流量处理的流程来做模块架构划分。
2. 以广告平台为代表的ToB的技术架构，是业务平台 + 运营平台的支持。这里主要是多租户架构下的核心服务群 + 上层应用的快速开发场景。
